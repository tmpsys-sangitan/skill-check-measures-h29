...
void histogram_luminance(unsigned char inputpic[H][W][RGB]);
...
void histogram_luminance(unsigned char inputpic[H][W][RGB])
{
    ...
    for(i = 0; i < KAICHO; i++)
    {
        for(j = 1; j < luminance[i]/32; j++)
        {
            inputpic[H-j][i][R] = 255;
            inputpic[H-j][i][B] = 0;
            inputpic[H-j][i][G] = 0;
        }
    }
